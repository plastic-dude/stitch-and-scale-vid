...
1. Pull and fetch the latest `origin/main` repository state. Read `references/publication-control.md` first, then read the newest remote approved brief or open remake order, exact script, claim ledger, shot list, production bible, platform matrix, caption map, audio plan, angle ledger, previous rejection notes, and `production-lessons/test-video-lessons.md`. The remote `origin/main` tree is authoritative; never discover work from a local Director branch, bundle, attachment, or hidden chat memory.
...
3. Resolve the work mode. If an open `REMAKE_REQUIRED` order exists, acknowledge it first in `orders/acknowledged/`, preserve the original filename/version, and use the order’s correction sequence as the minimum repair contract. Otherwise require a remote brief with status `approved-for-generation` and its matching script. Before declaring the queue empty, inspect the latest remote Director publication receipt, any `publication-blocked` record, and the single-flight state in `references/publication-control.md`. If `publication-blocked` exists, write one dependency-blocked escalation naming the local/remote commits and exact missing action; do not create another ordinary repetitive empty-queue record. If the active `plan_id`/`video_id` is already in progress or awaiting review, retry or resume that same contract and never select a second new brief.
...
5. Reserve the unique filename before generation. The reservation is idempotent: reuse the same `video_id` and reserved filename when retrying the same contract, and increment the version only for a documented Reviewer `REMAKE_REQUIRED` order:
...
15. Commit the production record and report the commit, one output filename, external media reference, research sources, QA result, source order ID if any, and unresolved risk. If blocked, report the remote commit inspected, exact paths checked, publication receipt or escalation record used, and the one dependency owner/action. After two consecutive no-eligible-contract runs while a local Director package is known to exist, create `PUBLICATION_ESCALATION_REQUIRED` once and stop repeating ordinary queue-starvation records. Stop after the one-video deliverable or one bounded escalation.

## Hard rejection rules

Reject or block any video that has obstructive captions, contradictory or hallucinated UI/text, illegible proof, mixed voices, generic copy for a high-stakes campaign, unsupported claims, unlicensed media, ambiguous filenames, missing checksums, wrong platform profile, missing external-storage metadata, or an unresolved Reviewer order. A local-only Director commit is not an eligible contract. Do not ask the Director to create a replacement idea when the real dependency is remote publication.
