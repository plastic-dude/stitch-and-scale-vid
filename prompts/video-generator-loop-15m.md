...
3. Resolve the work mode. If an open `REMAKE_REQUIRED` order exists, acknowledge it first in `orders/acknowledged/`, preserve the original filename/version, and use the order’s correction sequence as the minimum repair contract. Otherwise require a remote brief with status `approved-for-generation` and its matching script. Before declaring the queue empty, inspect the latest remote Director publication receipt and any `publication-blocked` record. If `publication-blocked` exists, write one dependency-blocked escalation naming the local/remote commits and exact missing action; do not create another ordinary repetitive empty-queue record.
