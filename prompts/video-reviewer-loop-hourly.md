...
The reviewer queue is stateful across runs. Persist the cursor at `state/video-reviewer-queue.yml` and commit it with every review or blocker. The state file must contain at least `last_scan_at`, `last_reviewed_video_id`, `last_reviewed_version`, `last_reviewed_at`, `last_outcome`, `next_cursor_key`, and `blocked_reason` when applicable. If the file does not exist, create it before the first review. Never infer continuation from chat memory alone.
...
Build one deterministic queue from the current repository and Drive state. Sort by: (1) open remake order priority, (2) newly generated VIDEO GENERATOR output since the cursor, (3) never-reviewed assets, (4) oldest eligible review timestamp, and (5) stable `video_id` plus version as a tie-breaker. Starting after `next_cursor_key`, choose the first eligible item in that order; wrap once only after checking every item after the cursor. If the queue has no eligible item, write one queue-blocking record, update `last_scan_at` and `blocked_reason`, and stop. After the one review or blocker, update the cursor to the actual item/outcome and set `next_cursor_key` to the next deterministic queue key. This is the mechanism that makes the next run continue where the previous run left off.
...
12. End after one bounded review or one clearly documented queue-blocking action. Before stopping, update and commit `state/video-reviewer-queue.yml` with the scan time, reviewed item/version, outcome, and next cursor. Report the research question, VIDEO GENERATOR output or video reviewed, score or blocker, verdict, remake order ID if any, and next queue item in this same chat. Do not launch another task, open another chat, or continue into a second video in the same firing.
