# Director Research — Artifact-Bound Terminal Decision and Low-Noise Holds

- **Research ID:** `RESEARCH-20260821-artifact-bound-terminal-low-noise-0940`
- **Run role:** Temporary Director/Planner
- **Firing:** 173
- **Run time:** 2026-08-21T09:40:00+01:00
- **Status:** Research-backed operational decision

## Fresh research question

What makes a terminal review decision sufficiently bound to the correct artifact, and how should recurring checks preserve safety without turning repeated hold records into duplicate production instructions?

## Current repository observation

The published Director playbook and `references/publication-control.md` were inspected before this decision. `origin/main` contains another queue-only hold after firing 172. V002 remains `video-ready-for-review`; no independent terminal Reviewer decision, owner action, `REMAKE_REQUIRED` order, or approved-for-generation brief exists. The single-flight lane remains closed.
...
NIST’s AI Risk Management Framework and Playbook emphasize accountable roles, documented processes, traceability, and ongoing monitoring. A terminal record should therefore identify the responsible role, the precise asset under review, the decision, and supporting evidence; process continuity alone cannot establish a quality outcome. Sources: [NIST AI Risk Management Framework](https://www.nist.gov/itl/ai-risk-management-framework), [NIST AI RMF 1.0](https://nvlpubs.nist.gov/nistpubs/ai/NIST.AI.100-1.pdf), and [NIST AI RMF Playbook](https://airc.nist.gov/airmf-resources/playbook/).

AWS callback-task and human-approval patterns pause a workflow until a corresponding unique callback returns explicit approval or rejection. This supports a fail-closed V002 lane: a queue append, timeout, reminder, silence, or generic acknowledgement cannot resume the waiting contract, and an artifact-mismatched callback cannot authorize V002. Sources: [AWS Step Functions — Integrating with Resources](https://docs.aws.amazon.com/step-functions/latest/dg/connect-to-resource.html), [AWS Step Functions — Human Approval](https://docs.aws.amazon.com/step-functions/latest/dg/tutorial-human-approval.html), and [AWS — Serverless Manual Approval Steps](https://aws.amazon.com/blogs/compute/implementing-serverless-manual-approval-steps-in-aws-step-functions-and-amazon-api-gateway/).

Google SRE guidance emphasizes actionable conditions, high signal, low noise, and deduplicating equivalent alerts. The Director should preserve a useful append-only record but should not create a new brief or production lane from a same-state queue hold. Sources: [Google SRE — Monitoring Distributed Systems](https://sre.google/sre-book/monitoring-distributed-systems/), [Google SRE — Practical Alerting](https://sre.google/sre-book/practical-alerting/), and [Google SRE — Alerting on SLOs](https://sre.google/workbook/alerting-on-slos/).

## Decision

Produce exactly one blocker stating that the newest queue-only hold remains non-terminal, requiring a unique independent Reviewer callback bound to exact V002 provenance and bounded owner escalation. Preserve immutable evidence and keep the single-flight lane closed.
